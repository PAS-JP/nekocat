...
pub fn sql_update_by_field(
        ...
        for col in &set_columns {
            let tok = fields
                ...
                .map(|ff| {
                    ...
                    quote! { (&self.#id) as &(dyn nekocat::tokio_postgres::types::ToSql + Sync) }
                })
                .unwrap();
            toks.push(tok);
        }
    ...
    vec_tokens
        .push(quote! { (&self.#ident) as &(dyn nekocat::tokio_postgres::types::ToSql + Sync) });
    ...
    quote! {
        impl #impl_block {
            pub async fn #upd_fn(
                &self,
                client: &impl nekocat::tokio_postgres::GenericClient
            ) -> Result<u64, nekocat::tokio_postgres::Error> {
