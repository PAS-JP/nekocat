...
pub fn derive_cipher(input: proc_macro::TokenStream) -> proc_macro::TokenStream {
    let input = parse_macro_input!(input as DeriveInput);
...
pub fn cipher(
    ...
    quote! {
        #[derive(
            nekocat::Cipher,
            nekocat::rkyv::Archive,
            nekocat::rkyv::Serialize,
            nekocat::rkyv::Deserialize
        )]
        #[rkyv(compare(PartialEq), derive(Debug))]
        #[rkyv(crate = nekocat::rkyv)]
        #input
    }
    .into()
}
...
pub fn derive_parser(input: proc_macro::TokenStream) -> proc_macro::TokenStream {
    ...
    let parser_app_code = parser_app(&input);
...
pub fn parser(
    ...
    quote! {
        #[derive(
            nekocat::Parser,
            nekocat::serde::Serialize,
            nekocat::serde::Deserialize,
            nekocat::rkyv::Archive,
            nekocat::rkyv::Serialize,
            nekocat::rkyv::Deserialize
        )]
        #[serde(crate = "nekocat::serde")]
        #[rkyv(compare(PartialEq), derive(Debug))]
        #[rkyv(crate = nekocat::rkyv)]
        #input
